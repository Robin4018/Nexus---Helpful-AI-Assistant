...
import { TerminalSquare, Mail, Lock, ArrowRight, Loader2, Eye, EyeOff } from 'lucide-react';
        ...
        >
            ...
            >
                <div className="text-center mb-6 sm:mb-8">
                    ...
                    <p className="text-sm sm:text-base text-muted-foreground mt-2">Sign in to your Nexus workspace</p>
                </div>
                ...
                <div className="bg-card/50 backdrop-blur-xl border border-border/50 rounded-2xl shadow-2xl overflow-hidden">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-5">
                            {error && (
                                <motion.div
                                    ...
                                    className="bg-destructive/10 text-destructive text-sm p-3 rounded-lg border border-destructive/20 text-center font-medium"
                                >
                                    {error}
                                </motion.div>
                            )}
                            ...
                            <button
                                type="submit"
                                disabled={loading}
                                className="w-full bg-primary text-primary-foreground font-bold py-3.5 rounded-xl hover:opacity-90 transition-all flex items-center justify-center gap-2 group shadow-lg shadow-primary/20 disabled:opacity-50"
                            >
                                {loading ? (
                    ...
                    <div className="bg-sidebar-accent/30 py-4 px-6 sm:px-8 border-t border-border/50 text-center">
                        <p className="text-sm text-muted-foreground">
                            New to Nexus? <Link to="/signup" className="text-primary font-bold hover:underline transition-all">Create account</Link>
                        </p>
                    </div>
                </div>
            </motion.div>
